const colors = [{
    ...
    "green": [
        ...
        ["darkseagreen", "#8fbc8b", "143, 188, 139", "115, 27%, 64%"],
...
for (let i = 0; i < colors.length; i++) {
    let category = colors[i];
    for (let key in category) {
        html += `<h3 class="mt-2">${key}</h3><table class="table fullwidth divide mt1"><thead><tr><th>CATEGROY</th><th>COLOR</th><th>HEX</th><th>RGB</th><th>HSL</th></tr></thead><tbody>`;
        let children = category[key];
        for (let j = 0; j < children.length; j++) {
            let obj = children[j];
...
function convert(val) {
    ...
    document.getElementById('table_preview').innerHTML = `<tr><td style="background-color:${hex}"></td><td>HEX</td><td>${hex}</td></tr>` + `<tr><td style="background-color:${hex}"></td><td>RGB</td><td>${color.toRGB()}</td></tr>` + `<tr><td style="background-color:${rgba}"></td><td>RGBA</td><td>${rgba}</td></tr>` + `<tr><td style="background-color:${rgba}"></td><td>ARGB</td><td>${color.toARGB()}</td></tr>` + `<tr><td style="background-color:${hex}"></td><td>HSL</td><td>${color.toHSL()}</td></tr>`;
}
convert('#6c928c');
