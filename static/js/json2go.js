function json2go(json, typename, flatten = true) {
	...
	function parseStruct(depth, innerTabs, scope, omitempty) {
		...
		if (flatten && depth >= 2) {
			...
			struct {\n`); ++innerTabs; const keys = Object.keys(scope);
				for (let i in keys) {
		...
		function getOriginalName(unique) {
			const reLiteralUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i const uuidLength = 36;
